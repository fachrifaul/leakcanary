...
class HprofRetainedHeapPerfTest {
  ...
  @Test fun `freeze retained memory through analysis steps of leak_asynctask_o`() {
    ...
    assertThat(retained after PARSING_HEAP_DUMP).isEqualTo(4.3 MB +-10 % margin)
    assertThat(retained after EXTRACTING_METADATA).isEqualTo(4.7 MB +-10 % margin)
    assertThat(retained after FINDING_RETAINED_OBJECTS).isEqualTo(4.8 MB +-10 % margin)
    assertThat(retained after FINDING_PATHS_TO_RETAINED_OBJECTS).isEqualTo(6.2 MB +-10 % margin)
    assertThat(retained after COMPUTING_NATIVE_RETAINED_SIZE).isEqualTo(6.2 MB +-10 % margin)
